...
halfSessionTable = readtable('../Data/params_half_best_2024-02-29.csv');
dataTable = readtable('../Data/all_parsed_data_2023-08-06.csv');
...
for rr = 1:numRhos
    ...
    for ss = 1:2
        ...
        ys = bData(Lr,7+ss);
        Lg = isfinite(xs) & isfinite(ys);
        plot(xs(Lg), ys(Lg), cat(2, 'k', sy{ss}))
        tstr = sprintf('%s, pn=%.2f', tstr, signrank(xs(Lg), ys(Lg)));
    end
    axis([-1 1 -1 1]);
    title(tstr)
end
